# Evidence

Evidence is part of the product, not an afterthought. Each phase report must contain Goal, Implementation, Verification commands, Results, relevant output, files changed and Known Limitations.

Required reports:

- `phase-0/report.md`
- `phase-1/runtime-report.md`
- `phase-1/persistence-report.md`
- `phase-2/prompt-context-report.md`
- `phase-2/compaction-report.md`
- `phase-3/replay-report.md`
- `phase-3/inspector-report.md`
- `phase-4/report.md`
- `final-report.md` (regenerated from clean-run output after the review cycle)
- `review-r1-r4/report.md` (post-MVP review improvements, P0 items R1–R4)
- `review-r5-r6-r8/report.md` (review improvements: token auditability, scenario suite, release verification)
- `review-r7/report.md` (optional OpenAI-compatible provider smoke path)
- `review-r9-r10/report.md` (Inspector polish, visual walkthrough with screenshots, portfolio narrative)
- `release-v0.1.0/report.md` (release close-out: smoke script, doc drift fixes, v0.1.0 tag)
- `portfolio-polish/report.md` (portfolio presentation: LICENSE, badges, screenshots, mypy scope, estimator calibration)
- `phase-6-1-task-runs/report.md` (real-provider task runs: provider factory, tool schemas, tools module, faithful continuation)
- `phase-6-2-background-runs/report.md` (202 + polling, bounded run queue, startup sweep)
- `phase-6-3-deployment/report.md` (bearer auth, CORS, healthz, Docker build + real container smoke)

Reports must show actual test/demo output; “implemented successfully” alone is not evidence. Keep secrets and API keys out of reports.

